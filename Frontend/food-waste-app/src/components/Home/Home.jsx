...
const Home = () => {
    ...
    useEffect(() => {
        const checkAuthentication = async () => {
            try {
                const response = await axios.get(`${process.env.VITE_API_HOST}/api/user/getLoggedInUser`, { withCredentials: true });
    ...
    useEffect(() => {
        if (user) {
            const fetchData = async () => {
                try {
                    const foodResponse = await axios.get(`${process.env.VITE_API_HOST}/api/foodItem/getByUserId/${user.user.id}`, { withCredentials: true });
                    setFoodItems(foodResponse.data);

                    const friendsResponse = await axios.get(`${process.env.VITE_API_HOST}/api/follows/getFriends/${user.user.id}`, { withCredentials: true });
                    const friendPromises = friendsResponse.data.map(async (friend) => {
                        const userResponse = await axios.get(`${process.env.VITE_API_HOST}/api/user/get/${friend.followeeId}`);
                        return {
    ...
    const handleAddFood = async () => {
        ...
        try {
            if (user) { 
                ...
                const response = await axios.post(`${process.env.VITE_API_HOST}/api/foodItem/create`, newFood, { withCredentials: true });
    ...
    const handleRemoveFood = async (id) => {
        try {
            await axios.delete(`${process.env.VITE_API_HOST}/api/foodItem/delete/${id}`, { withCredentials: true });
    ...
    const handleFollow = async () => {
        try {
            const users = await axios.get(`${process.env.VITE_API_HOST}/api/user/get`);
            const foundUser = users.data.find(otherUser => otherUser.email === searchTerm);
            if (foundUser) {
                const response = await axios.post(`${process.env.VITE_API_HOST}/api/follows/create`, { 
    ...
    const handleRemoveFriend = async (friendId) => {
        try {
            await axios.delete(`${process.env.VITE_API_HOST}/api/follows/delete/${user.user.id}/${friendId}`, { withCredentials: true });
